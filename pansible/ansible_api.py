...
class MyRunner(MyInventory):
    ...
    def results(self):
        """
        {'dark': {}, 'failed': {'localhost': ''}, 'ok': {'localhost': ''}}
        """
        result = {'dark':{},'failed': {}, 'ok': {}}
        dark = self.results_raw.get('dark')
        contacted = self.results_raw.get('contacted')
        if dark:
            for host, info in dark.items():
                result['dark'][host] = info.get('msg')

        if contacted:
            for host, info in contacted.items():
                if info.get('invocation').get('module_name') in ['raw', 'shell', 'command', 'script', 'async_status']:
                    if info.get('rc') == 0:
...
class MyTask(MyRunner):
    ...
    def copy(self,dest,src):
        ...
        module_args = 'dest=%s, src=%s, backup="yes", directory_mode' % (dest, src)
        result = self.run("copy",module_args,become=True)
        return result
    ...
    def add_user(self, username, password=''):
        ...
        cmd5 = 'chattr -i /home/%s/.ssh/authorized_keys' % username
        ...
        self.run("shell", cmd5, become=True)
    ...
    def del_user(self, username):
        ...
        #module_args = 'name=%s state=absent force=yes' % username
        cmd1 = 'chattr -i /etc/group'
        cmd2 = 'chattr -i /etc/gshadow'
        cmd3 = 'chattr -i /etc/passwd'
        cmd4 = 'chattr -i /etc/shadow'
    ...
    def hello(self,module_args):
        try:
            result = self.run("command", module_args, become=True)
...
if __name__ == "__main__":
    ...
    resource = [{"hostname": "127.0.0.1","ip":'127.0.0.1', "port": "22", "username": "xiaolong", "password": "258258"},{"hostname": "192.168.186.132","ip":'192.168.186.132', "port": "22", "username": "wangxiaolong", "password": "vitamin20"},{"hostname": "192.168.186.136","ip":'192.168.186.136', "port": "22", "username": "wangxiaolong", "password": "vitamin20"}]
    #resource = [{"hostname":"192.168.186.132","ip":"192.168.186.132","port":"22","username":"wangxiaolong","password":"vitamin20"}]
    cmd = MyTask(resource)
    result = cmd.hello('cat text.txt')
    print result.get('dark').items()
    print result.get('failed').items()
    print result.get('ok').items()
